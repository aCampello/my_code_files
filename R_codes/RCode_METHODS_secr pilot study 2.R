...
captures<-read.table(file = "SECRcaptures_trapIDformat.txt", sep='\t', header = TRUE)
trapfilemulti<-read.table(file = "trapfile.txt", sep='\t', header = T)
...
foxCH <- make.capthist(captures,trapfile, detector = "count",
...
foxCH <- make.capthist(captures, trapfilemulti, fmt= c("trapID", "XY"), covnames = "Sex", "Status")

summary(foxCH)


######DETECTOR TYPE "COUNT" NEEDS TOTAL NO.VISITS THAT DAY BY ALL FOXES##########

######DETECTOR TYPE "MULTI" NEEDS TOTAL NO.DIFFERENT FOXES DETECTED THAT DAY #####
